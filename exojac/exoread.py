...
class ExodusIIReader(object):
    ...
    def __init__(self, filepath):
        ...
        self._nvaridx = dict([(k, i) for (i, k) in enumerate(self.node_var_names)])
        self._evaridx = dict([(k, i) for (i, k) in enumerate(self.elem_var_names)])
        self._gvaridx = dict([(k, i) for (i, k) in enumerate(self.glob_var_names)])
        pass
    ...
    def num_el_in_blk(self, elem_blk_id):
        return self.db.dimensions[DIM_NUM_EL_IN_BLK(elem_blk_id)]

    def el_type_in_blk(self, elem_blk_id):
        return self.db.variables[VAR_CONN(elem_blk_id)].elem_type

    def num_nod_per_el_in_blk(self, elem_blk_id):
        return self.db.dimensions[DIM_NUM_NOD_PER_EL(elem_blk_id)]

    def num_attr_in_blk(self, elem_blk_id):
        return self.db.dimensions.get(DIM_NUM_ATT_IN_BLK(elem_blk_id), 0)
    ...
    def get_names(self, item):
        try:
            return chara_to_text(self.db.variables[item].data, aslist=True)
        except KeyError:
            return []

    @property
    def glob_var_names(self):
        return self.get_names(VAR_NAME_GLO_VAR)

    @property
    def elem_var_names(self):
        return self.get_names(VAR_NAME_ELE_VAR)

    @property
    def node_var_names(self):
        return self.get_names(VAR_NAME_NOD_VAR)

    def nodal_var_index(self, name):
        try:
            return self._nvaridx[name]
        except KeyError:
            raise ExodusIIFileError("{0}: nodal var not found".format(name))

    def global_var_index(self, name):
        try:
            return self._gvaridx[name]
        except KeyError:
            raise ExodusIIFileError("{0}: global var not found".format(name))

    def elem_var_index(self, name):
        try:
            return self._evaridx[name]
        except KeyError:
            raise ExodusIIFileError("{0}: element var not found".format(name))
    ...
    def elem_blk_num(self, elem_blk_id):
        try:
            return np.where(self.elem_blk_ids == elem_blk_id)[0][0] + 1
        except ValueError:
            raise ExodusIIFileError("{0}: not an element block".format(elem_blk_id))
    ...
    @property
    def node_num_map(self):
        return self.get_node_num_map()
    ...
    def get_coord(self, idx=None):
        ...
        """
        if idx is not None:
            return self.db.variables[PX_VAR_COORDS(idx)].data[:]

        coords = []
        for i in range(self.num_dim):
            coords.append(self.db.variables[PX_VAR_COORDS(i)].data[:])
    ...
    def get_elem_block(self, elem_blk_id):
        ...
        """
        elem_type = self.db.variables[VAR_CONN(elem_blk_id)].elem_type
        num_elem_this_blk = self.num_el_in_blk(elem_blk_id)
        num_nodes_per_elem = self.num_nod_per_el_in_blk(elem_blk_id)
        num_attr = self.num_attr_in_blk(elem_blk_id)
    ...
    def get_elem_conn(self, elem_blk_id, disp=0):
        ...
        """
        var = self.db.variables[VAR_CONN(elem_blk_id)]
        if not disp:
            return var.data
    ...
    def get_elem_num_map(self):
        ...
        """
        try:
            return self.db.variables["elem_num_map"].data[:]
        except KeyError:
            return np.arange(1, self.num_elem+1, dtype=np.int32)

    def get_node_num_map(self):
        try:
            return self.db.variables["node_num_map"].data[:]
        except KeyError:
            return np.arange(1, self.num_nodes+1, dtype=np.int32)
    ...
    def get_map(self):
        ...
        """
        try:
            return self.db.variables["elem_map"].data[:]
        except KeyError:
            return np.arange(1, self.num_elem+1, dtype=np.int32)
    ...
    def get_glob_var_time(self, glob_var):
        ...
        """
        i = self.global_var_index(glob_var)
    ...
    def get_nodal_var(self, step, nodal_var):
        ...
        if step == 0:
            step = 1
        i = self.nodal_var_index(nodal_var)
    ...
    def get_nodal_var_time(self, nodal_var, node_num):
        ...
        """
        i = self.nodal_var_index(nodal_var)
        return self.db.variables[VAR_NOD_VAR_NEW(i+1)].data[:, node_num-1]
    ...
    def get_elem_var(self, step, elem_var):
        ...
        """
        i = self.elem_var_index(elem_var)
        elem_var = []
        for elem_blk_id in self.elem_blk_ids:
            n = self.db.dimensions[DIM_NUM_EL_IN_BLK(elem_blk_id)]
            name = VAR_ELEM_VAR(i+1, elem_blk_id)
    ...
    def get_elem_var_time(self, elem_var, elem_num, elem_blk_id=None):
        ...
        name = VAR_ELEM_VAR(i+1, elem_blk_id)
        return self.db.variables[name].data[:, e-1]
    ...
    def get_node_set(self, node_set_id):
        """Reads the node list for a single node set.

        """
        return self.db.variables[VAR_NODE_NS(node_set_id)].data

    def get_side_set(self, side_set_id):
        """Reads the node list for a single node set.

        """
        elems = self.db.variables[VAR_ELEM_SS(side_set_id)]
        sides = self.db.variables[VAR_SIDE_SS(side_set_id)]
        return np.array([(elem, sides[i]) for (i, elem) in enumerate(elems)])
