...
class ExodusIIReader(object):
    ...
    def __init__(self, filepath):
        ...
        self._times = self.get_all_times()
        self._steps = range(self._times.shape[0])
        pass
    ...
    def _get_var_ids(self):
        ...
        var_id_idx_map[PX_VAR_NOD] = dict((j, i)
                                  for (i, j) in enumerate(self.nod_var_names))
    ...
    def get_step(self, time):
        """Get the step associated with time

        """
        step = np.interp(time, self._times, self._steps)
    ...
    def summary(self):
        ...
        S.append("Nodal Variables: {0}".format(", ".join(self.nod_var_names)))
...
class ExodusIIManager(object):
    ...
    def put_init(self, runid, num_dim, coords, conn, elem_blks,
                 node_sets, side_sets, all_element_data, elem_num_map,
                 title=None, d=None):
        ...
        z = np.zeros(self.num_nodes)
        self.exofile.put_coord(coords[:], y, z)

        # write element order map
        self.exofile.put_elem_num_map([_+1 for _ in elem_num_map])
    ...
    def snapshot(self, u):
        ...
        temp = 0.
        dtemp = 0.
        stran = np.zeros(self.num_elem)
        stress = np.zeros(self.num_elem)
        ...
        for (i, ebid) in enumerate(elem_blk_ids):
            elems_this_blk = elems_per_block[i]
            neeb = len(elems_this_blk)
            data = np.column_stack((stress[elems_this_blk],
                                    stran[elems_this_blk],
                                    dstran[elems_this_blk]))
            for k in range(self.num_elem_vars):
