...
from exolite import ExodusIIFile
...
class Plot2D(tapi.HasTraits):
    ...
    def find_time_index(self):
        # tjf: prolly not needed
        list_of_diffs = [abs(x) for x in self.plot_data[0][:, 0]]
    ...
    def create_plot(self, x, y, z, plot_name):
        ...
        # Plot the node variable
        self.container.plot(
            ("x " + plot_name, "y " + plot_name, "color"),
            type="cmap_scatter",
            name=plot_name,
            marker="circle",
            color_mapper=hot,
...
def create_model_plot(sources, handler=None, metadata=None):
    ...
    view = tuiapi.View(tuiapi.HSplit(
        ...
        tuiapi.VGroup(
            tuiapi.Item('Plot_Data', show_label=False, width=800, height=568,
                        springy=True, resizable=True),
            tuiapi.Item('Step', editor=tuiapi.RangeEditor(low_name='low_step',
                                                          high_name='high_step',
                                                          format='%d',
                                                          label_width=28,
                                                          mode='slider')),
...
def loadcontents(filepath, step=0):
    exof = ExodusIIFile(filepath, "r")
    nod_var_names = exof.nod_var_names
    data = [exof.get_coord(0)]
    for nod_var_name in nod_var_names:
        data.append(exof.get_nodal_var(step, nod_var_name))
    data = np.transpose(np.array(data))
    head = ["X"] + nod_var_names
    exof.close()
    return head, data
...
def get_sorted_fileinfo(filepaths, step=0):
    ...
    for filepath in filepaths:
        ...
        fhead, fdata = loadcontents(filepath, step=step)
        if not np.any(fdata):
