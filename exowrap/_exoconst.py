# build directory to mimic that of Numpy's distutils
import os
import sys
from distutils.util import get_platform
D = os.path.dirname(os.path.realpath(__file__))
major, minor = sys.version_info[0:2]
platform_info = "{0}-{1}.{2}".format(get_platform(), major, minor)
prefix = os.path.join(D, "build", platform_info)
pyexodusii_library = os.path.join(prefix, "pyexodusii.so")
