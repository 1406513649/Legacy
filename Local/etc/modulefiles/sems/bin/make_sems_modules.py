...
for (compiler_name, compiler_versions) in COMPILERS.items():
    for compiler_version in compiler_versions:
        ...
        if compiler_name == 'python':
            source = os.path.join(ROOT_D, 'default_sems_python.py')
        else:
            source = os.path.join(ROOT_D, 'default_sems_compiler.py')
        module = os.path.join(module_d, compiler_version + '.py')
...
for (util_name, util_versions) in UTILITY_VERSIONS.items():
    for util_version in util_versions:
        ...
        if not os.path.isdir(module_d):
            os.makedirs(module_d)
        source = os.path.join(ROOT_D, 'default_sems_util.py')
        module = os.path.join(module_d, util_version + '.py')
        shutil.copyfile(source, module)
...
for (key, path) in TPL_BASE_VERSIONS.items():
    ...
    if not os.path.isdir(module_d):
        os.makedirs(module_d)
    source = os.path.join(ROOT_D, 'default_sems_compiler_dep.py')
    module = os.path.join(module_d, tpl_version + '.py') #'base.py')
    shutil.copyfile(source, module)
...
for (key, path) in TPL_MPI_VERSIONS.items():
    ...
    if 'parmetis' not in tpl:
        source = os.path.join(ROOT_D, 'default_sems_mpi_dep.py')
        module = os.path.join(module_d, tpl_version+'.py') # 'parallel.py')
        shutil.copyfile(source, module)
        print(tpl, tpl_version, end=': ')
        print(module)
    else:
        source = os.path.join(ROOT_D, 'parmetis_sems_mpi_dep.py')
        #for x in ('', '64bit_', '32bit_'):
        for x in ('', '_64', '_32'):
            #module = os.path.join(module_d, x + 'parallel.py')
            module = os.path.join(module_d, tpl_version+x+'.py')
            shutil.copyfile(source, module)
            print(tpl, tpl_version, end=': ')
            print(module)
    ...
    if not os.path.isdir(module_d):
        os.makedirs(module_d)
    source = os.path.join(ROOT_D, 'default_sems_mpi.py')
    module = os.path.join(module_d, mpi_version+'.py')
    shutil.copyfile(source, module)
    print(key[-3], end=': ')
    print(module)
