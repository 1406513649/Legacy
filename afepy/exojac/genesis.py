...
class Genesis(object):
    ...
    def __init__(self, filename, d=None):
        ...
        self.filepath = os.path.join(d, basename)
    ...
    def put_elem_block(self, elem_blk_id, elem_type, num_elem_this_blk,
        ...
        i = self.counter[PX_ELEM_BLK] + 1
        if self.counter[PX_ELEM_BLK] > num_elem_blk:
            raise ExodusIIFileError("number element blocks exceeded")

        # dimensioning
        self.db.createDimension(DIM_NUM_EL_IN_BLK(i), num_elem_this_blk)
        self.db.createDimension(DIM_NUM_NOD_PER_EL(i), num_nodes_per_elem)

        # store the element block ID
        self.db.variables[VAR_ID_EL_BLK][i-1] = elem_blk_id

        # set up the element block connectivity
        self.db.createVariable(VAR_CONN(i), DTYPE_INT,
                               (DIM_NUM_EL_IN_BLK(i), DIM_NUM_NOD_PER_EL(i)))
        setattr(self.db.variables[VAR_CONN(i)], "elem_type",
                elem_type.upper())
        conn = np.zeros((num_elem_this_blk, num_nodes_per_elem))
        self.db.variables[VAR_CONN(i)][:] = conn

        # element block attributes
        if num_attr:
            self.db.createDimension(DIM_NUM_ATT_IN_BLK(i), num_attr)

            self.db.createVariable(VAR_ATTRIB(i), DTYPE_FLT,
                               (DIM_NUM_EL_IN_BLK(i), DIM_NUM_ATT_IN_BLK(i)))
            self.db.variables[VAR_ATTRIB(i)][:] = np.zeros(num_attr)

            self.db.createVariable(VAR_NAME_ATTRIB(i), DTYPE_TXT,
                                   (DIM_NUM_ATT_IN_BLK(i), DIM_STR))
            attr_name = " " * MAX_STR_LENGTH
            self.db.variables[VAR_NAME_ATTRIB(i)][:] = attr_name
    ...
    def put_elem_conn(self, elem_blk_id, blk_conn):
        ...
        i = self.get_elem_blk_num(elem_blk_id)

        # dimensions
        dim_e = DIM_NUM_EL_IN_BLK(i)
        num_elem_this_blk = self.db.dimensions[dim_e]

        dim_n = DIM_NUM_NOD_PER_EL(i)
        ...
        # connectivity
        self.db.variables[VAR_CONN(i)][:] = blk_conn + 1
        return
    ...
    def put_elem_attr(self, elem_blk_id, attr):
        ...
        dim_a = DIM_NUM_ATT_IN_BLK(i)
        num_attr_this_block = self.db.dimensions[dim_a]

        # put the attribute
        self.db.variables[VAR_ATTRIB(i)][:] = attr
        return
    ...
    def put_node_set_param(self, node_set_id, num_nodes_in_set,
        ...
        # store node set ID
        i = self.counter[PX_NODE_SET] + 1
        self.db.variables[VAR_NS_IDS][i-1] = int(node_set_id)

        self.db.createDimension(DIM_NUM_NOD_NS(i), num_nodes_in_set)
        self.db.createVariable(VAR_NODE_NS(i), DTYPE_INT, (DIM_NUM_NOD_NS(i),))

        if num_dist_fact_in_set:
            self.db.createDimension(DIM_NUM_DF_NS(i), num_dist_fact_in_set)
            self.db.createVariable(VAR_FACT_NS(i), DTYPE_FLT, (DIM_NUM_NOD_NS(i),))

        self.db.variables[VAR_NS_STAT][i-1] = 1
    ...
    def put_node_set(self, node_set_id, node_set_node_list):
        ...
        """
        i = self.get_node_set_num(node_set_id)
        nodes = node_set_node_list + 1
        self.db.variables[VAR_NODE_NS(i)][:] = nodes
        return
    ...
    def put_node_set_dist_fact(self, node_set_id, node_set_dist_fact):
        ...
        """
        i = self.get_node_set_num(node_set_id)
        dim = self.db.dimensions[DIM_NUM_DF_NS(i)]
        if len(node_set_dist_fact) != dim:
            raise ExodusIIFileError("len(node_set_dist_fact) incorrect")
        self.db.variables[VAR_FACT_NS(i)][:] = node_set_dist_fact
    ...
    def put_side_set_param(self, side_set_id, num_sides_in_set,
        ...
        i = self.counter[PX_SIDE_SET] + 1

        # store side set ID
        self.db.variables[VAR_SS_IDS][i-1] = int(side_set_id)

        self.db.createDimension(DIM_NUM_SIDE_SS(i), num_sides_in_set)
        self.db.createVariable(VAR_SIDE_SS(i), DTYPE_INT, (DIM_NUM_SIDE_SS(i),))
        self.db.createVariable(VAR_ELEM_SS(i), DTYPE_INT, (DIM_NUM_SIDE_SS(i),))

        if num_dist_fact_in_set:
            self.db.createDimension(DIM_NUM_DF_SS(i), num_dist_fact_in_set)
            self.db.createVariable(VAR_FACT_SS(i), DTYPE_FLT, (DIM_NUM_DF_SS(i),))

        self.db.variables[VAR_SS_STAT][i-1] = 1
    ...
    def put_side_set(self, side_set_id, side_set_elem_list,
        ...
        """
        i = self.get_side_set_num(side_set_id)
        sides = side_set_side_list + 1
        elems = side_set_elem_list + 1
        self.db.variables[VAR_SIDE_SS(i)][:] = sides
        self.db.variables[VAR_ELEM_SS(i)][:] = elems
        return
    ...
    def put_side_set_dist_fact(self, side_set_id, side_set_dist_fact):
        ...
        """
        i = self.get_side_set_num(side_set_id)
        dim = self.db.dimensions[DIM_NUM_DF_SS(i)]
        if len(side_set_dist_fact) != dim:
            raise ExodusIIFileError("len(side_set_dist_fact) incorrect")
        self.db.variables[VAR_FACT_SS(i)][:] = side_set_dist_fact
    ...
    def get_elem_blk_num(self, elem_blk_id):
        return getidx(self.db.variables[VAR_ID_EL_BLK].data, elem_blk_id) + 1
    def get_side_set_num(self, side_set_id):
        return getidx(self.db.variables[VAR_SS_IDS].data, side_set_id) + 1
    def get_node_set_num(self, node_set_id):
        return getidx(self.db.variables[VAR_NS_IDS].data, node_set_id) + 1

def getidx(a, item):
    return np.where(a == item)[0][0]
