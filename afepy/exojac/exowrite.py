...
class ExodusIIWriter(Genesis):
    ...
    def __init__(self, filename, d=None):
        ...
        """
        super(ExodusIIWriter, self).__init__(filename, d=d)
        ...
        for (i, var_name) in enumerate(var_names):
            if var_type == PX_VAR_GLO:
                ...
                for elem_blk_id in self.elem_blk_ids:
                    j = self.get_elem_blk_num(elem_blk_id)
                    self.db.createVariable(VAR_ELEM_VAR(i+1, j), DTYPE_FLT,
                                           (DIM_TIME, DIM_NUM_EL_IN_BLK(j)))
    ...
    def put_nodal_var(self, time_step, nodal_var_index, num_nodes,
        ...
        """
        name = VAR_NOD_VAR_NEW(nodal_var_index+1)
    ...
    def put_elem_var(self, time_step, elem_var_index, elem_blk_id,
        ...
        name = VAR_ELEM_VAR(elem_var_index+1, i)
