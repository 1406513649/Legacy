...
import src.fem.mesh.mesh as femesh
from src.fem.io.logger import Logger
from src.fem.mesh.mesh_generators import gen_coords_conn_from_inline
from src.fem.io.io import IOManager
from src.base.errors import UserInputError
from src.base.function_builder import build_lambda, build_interpolating_function
from src.base.inpparse import UserInputParser
...
class FEModel(object):
    ...
    def __init__(self, runid, control, mesh):
        ...
        self.logger = Logger(runid, control[8])
    ...
    def setup_exodus_file(self):
        ...
        for (blk_id, blk_els) in self.mesh.element_blocks().items():
            elem_type = self.mesh.elements(blk_els[0]).name
            if elem_type == "RVE":
                elem_type = self.mesh.elements(blk_els[0]).parent.name
        ...
        self.io = IOManager(self.runid, self.dim, self.mesh.coords(),
    ...
    def from_input_string(runid, inp, rundir=None, verbosity=None):
        ...
        if implicit:
            import src.fem.core.lento as lento
            return lento.Lento(runid, ui.control, mesh)
        else:
            import src.fem.core.veloz as veloz
