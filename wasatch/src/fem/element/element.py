...
import src.base.utilities as utils
from src.base.tensor import asarray, asmatrix, reduce_map, reduce_map_C
from src.base.tensor import axialv, axialt
from src.base.tensor import NSYMM, NTENS, I3x3, I6, I9
from src.base.errors import UserInputError, WasatchError
...
class Element(object):
    ...
    def von_neumann(self, time, coords, tractions):
        ...
        xilist = self.bndry.gauss_coords
        w = self.bndry.gauss_weights

        ndof, nfnodes = self.ndof, self.nface_nodes()
        for intpt in range(self.bndry.ngauss):

            # Compute shape functions and derivatives wrt local coords
            xi = xilist[intpt]
            N = self.bndry.calc_shape(xi)
            dNdxi = self.bndry.calc_shape_deriv(xi)
    ...
    def update_state(self, t, dt, coords, u, du):
        ...
        # If this is an RVE, update internal state
        if self.rve_model is not None:
            save = self.data[1][:]
            self.update_rve_state(t, dt, coords, u, du)
            self.data[1] = save[:]
